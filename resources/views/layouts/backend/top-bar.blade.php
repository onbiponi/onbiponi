			...
			<ul class="nav navbar-nav ml-auto">
				...
				<li class="nav-item messages-menu" id="app">
					<a href="{{ route('chats.index') }}" class="nav-link py-0">
						...
						@auth
						<chat-counter v-bind:user='{{ json_encode($user) }}' v-bind:partner='@if(isset($partner)) {!! json_encode($partner) !!} @else "{}" @endif' v-bind:total_unread_message="{{ $total_unread_message ?? 0 }}"></chat-counter>
						@endauth
					</a>
				</li>
